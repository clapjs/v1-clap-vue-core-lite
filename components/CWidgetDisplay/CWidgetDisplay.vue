<template>
    <div>
        ...
        <template v-else-if="widget === 'Select'">
            <span v-if="!widgetConfig.hasOwnProperty('multiple')||!widgetConfig.multiple"  :style="{ color: range[0]&&range[0].color?range[0].color:'#000' }">{{range[0]&&range[0].name }}</span>
            <span v-else><span v-for="(tag,index) of range" :key="index" :style="{ color: tag.color?tag.color:'#000' }">{{ tag.name }}</span></span>
        </template>
